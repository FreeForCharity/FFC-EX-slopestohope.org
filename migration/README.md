...
- Build: the validated static export contains 29 publishable captured routes, generated `robots.txt` and `sitemap.xml`, and the `/staff/` redirect. Empty Community Across America tag archives, four retired GiveWP workflow pages, and the approved title-only policy shells are excluded.
