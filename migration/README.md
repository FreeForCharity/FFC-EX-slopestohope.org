...
> Local-route safeguard: source recapture preserves owner-authored routes such as `/coosummit26/`, restores their checked-in HTML after the WordPress pass, and merges them into the generated inventory. This keeps them in route validation and the generated sitemap.

## Verification completed

- Static validation: all eleven published routes pass; retained source wording is preserved except for the explicitly approved homepage display adaptations, all mapped internal links and local assets resolve, intentional external destinations match the capture, and no `.com` asset/page runtime URLs remain.
- Full browser audit: the current eleven published routes pass at 1440 px and 390 px with no missing assets, legacy requests, external failures, JavaScript errors, overflow, or broken images. The Newsletter, Contact, and COO Summit HubSpot form frames render at both widths.
- Interaction audit: reduced-motion hero behavior, normal-motion auto-advance, independent hover and keyboard-focus pause/resume behavior, overlapping hover/focus behavior, desktop and mobile newsletter anchors and invalid-email handling, donation and pledge destinations, Gallery lightbox open/next/Escape behavior, Contact form fields and required-field validation, and mobile menu open/navigate/reopen/close behavior pass.
- Build: the validated static export contains eleven published routes, generated `robots.txt` and `sitemap.xml`, and the `/staff/` redirect. Empty tag and author archives, two empty ElementsKit internal-content routes, the empty Volunteer shell, Community Across America tag archives, four retired GiveWP workflow pages, and the approved title-only policy shells are excluded.
...
- The final local browser audit passes all eleven published routes at 1440 px and 390 px with no missing assets, legacy requests, external failures, JavaScript errors, overflow, or broken images. All three HubSpot form frames render at both widths, and the final interaction rerun passes reduced-motion behavior, form validation, donation destinations, Gallery behavior, and mobile navigation.
