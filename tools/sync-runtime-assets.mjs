...
const MAX_ROUNDS = Number((args.find(a => a.startsWith('--max-rounds=')) || '').replace('--max-rounds=', '')) || 6;

const PAGE_PATHS = [
  '/', '/our-story/', '/team/', '/gallery/', '/contact-us/', '/donors/',
  '/faq/', '/volunteer/', '/donor-dashboard/', '/donation-confirmation/',
  '/donation-failed/', '/test-donate/', '/privacy-policy-2/', '/terms-of-service-2/',
];
...
async function collectMisses(base, browser) {
  ...
  for (const path of PAGE_PATHS) {
    ...
    } catch { /* a page that fails to load simply yields no misses */ }
