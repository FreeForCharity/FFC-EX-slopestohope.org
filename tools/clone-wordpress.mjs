...
// The clone is the Next static export's public/ directory: `next build`
// copies it verbatim into out/, which is what Pages serves.
...
const PAGES = [
  ...
  { slug: 'volunteer', out: 'volunteer/index.html', mode: 'shell', title: 'Volunteer' },
  {
    ...
    out: 'donor-dashboard/index.html',
    mode: 'shell',
    title: 'Donor Dashboard',
  },
  {
    slug: 'donation-confirmation',
    out: 'donation-confirmation/index.html',
    mode: 'shell',
    title: 'Donation Confirmation',
  },
  {
    slug: 'donation-failed',
    out: 'donation-failed/index.html',
    mode: 'shell',
  ...
  { slug: 'test-donate', out: 'test-donate/index.html', mode: 'shell', title: 'Donate' },
  {
    ...
    out: 'privacy-policy-2/index.html',
    mode: 'shell',
    title: 'Privacy Policy',
  },
  {
    slug: 'terms-of-service-2',
    out: 'terms-of-service-2/index.html',
    mode: 'shell',
 ...
 */
const REDIRECTS = [{ from: 'staff/index.html', to: '/team/' }]
...
function collectLegacyUrls(text) {
  ...
  const escaped = new RegExp(
    `https?:\\\\/\\\\/(?:www\\.)?slopestohope\\.com((?:\\\\/${URL_CHAR}*)*)`,
    'g'
  )
  let m
...
function replaceElementorRuntime(html) {
  ...
  // is replaced.
  out = out.replace(/<\/body>/i, '<script src="/assets/js/site.js"></script>\n</body>')
...
async function main() {
  const selected = ONLY.length ? PAGES.filter((p) => ONLY.includes(p.slug || 'home')) : PAGES
