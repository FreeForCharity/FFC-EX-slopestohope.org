 ...
 *
 * Usage:
 *   node tools/verify-no-legacy.mjs                 # serves the repo itself
...
const PAGES = [
  ...
  { path: '/donation-confirmation/', name: 'donation-confirmation', expect: [], forbid: ['[give_receipt]'] },
  { path: '/donation-failed/', name: 'donation-failed', expect: [] },
  { path: '/test-donate/', name: 'test-donate', expect: [], forbid: ['[give_form'] },
  { path: '/privacy-policy/', name: 'privacy-policy', expect: ['Privacy Policy', 'Slopes to Hope'], forbid: ['Free For Charity', 'freeforcharity.org'] },
  { path: '/cookie-policy/', name: 'cookie-policy', expect: ['Cookie Policy'], forbid: ['Free For Charity', 'freeforcharity.org'] },
  { path: '/terms-of-service/', name: 'terms-of-service', expect: ['Terms of Service'], forbid: ['Free For Charity', 'freeforcharity.org'] },
...
async function main() {
  ...
  for (const page of PAGES) {
    ...
    try {
      ...
      if (page.redirectsTo) {
        ...
        if (got !== page.redirectsTo) problems.push(`redirect went to ${got}, expected ${page.redirectsTo}`);
      }
    ...
    results.push({ page, status, problems, legacyHits, failedRequests, localMissing, consoleErrors });
    await ctx.close();
  }
  ...
  for (const r of results) {
    ...
    for (const u of r.failedRequests.slice(0, 3)) console.log(`        third-party unreachable: ${u}`);
  }
